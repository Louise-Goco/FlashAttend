...
const Profile = () => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        try {
          ...
          setFormData({
            studentId: data.studentId || data.identifier || "",
  ...
  const isFaculty = formData.role === 'Faculty' || formData.role === 'Admin';
  const themeColor = isFaculty ? "#4f46e5" : "#0d6efd";
  const themeGradient = isFaculty 
    ? "linear-gradient(135deg, #4f46e5 0%, #7c3aed 100%)" 
    : "linear-gradient(135deg, #0d6efd 0%, #6610f2 100%)";
  const avatarGradient = isFaculty
      ...
      <div className="row justify-content-center">
        <div className="col-lg-10">
          ...
          <div
            ...
            style={{ height: "180px", background: themeGradient }}
          >
            <div className="position-absolute bottom-0 start-0 w-100 p-4" style={{ background: "linear-gradient(to top, rgba(0,0,0,0.6) 0%, rgba(0,0,0,0) 100%)" }}>
              <h2 className="text-white fw-bold mb-0">{isFaculty ? 'Faculty Profile' : 'Student Profile'}</h2>
            ...
            <div className="col-md-4">
              <div className="card border-0 shadow-sm rounded-4 h-100">
                <div className="card-body text-center p-4">
                  ...
                  <p className="fw-semibold mb-3" style={{ color: themeColor }}>
                    {formData.studentId ? `${isFaculty ? 'Faculty ID' : 'Student ID'}: ${formData.studentId}` : "ID Loading..."}
                  </p>
                  <hr className="my-4 text-muted opacity-25" />
                  <div className="text-start">
                    <div className="d-flex align-items-center mb-3">
                      <div className="p-2 rounded-circle me-3" style={{ backgroundColor: `${themeColor}20`, color: themeColor }}><Mail size={18} /></div>
                      <span className="text-muted small text-truncate">{formData.email || 'No email provided'}</span>
                    </div>
                    <div className="d-flex align-items-center mb-3">
                      <div className="p-2 rounded-circle me-3" style={{ backgroundColor: `${themeColor}20`, color: themeColor }}><Phone size={18} /></div>
                      <span className="text-muted small">{formData.phone || 'No phone provided'}</span>
                    </div>
                    <div className="d-flex align-items-center mb-3">
                      <div className="p-2 rounded-circle me-3" style={{ backgroundColor: `${themeColor}20`, color: themeColor }}><Calendar size={18} /></div>
              ...
              <div className="card border-0 shadow-sm rounded-4">
                <div className="card-body p-5">
                  <h5 className="fw-bold mb-4 d-flex align-items-center gap-2">
                    <User style={{ color: themeColor }} size={20} /> Personal Information
                  </h5>
                  ...
                  <form onSubmit={handleSubmit}>
                    {/* ID — always read-only */}
                    <div className="mb-4">
                      <label className="form-label text-muted small fw-bold text-uppercase">{isFaculty ? 'Faculty ID Number' : 'Student ID Number'}</label>
                      <input
                        type="text"
                      ...
                      {isEditing ? (
                        <>
                          ...
                          <button
                            ...
                            disabled={isSaving}
                            style={{ transition: "all 0.3s ease", background: themeGradient, border: "none" }}
                          >
                            {isSaving ? (
                              <>
